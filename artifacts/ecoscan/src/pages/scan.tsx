...
import { UKLocationSearch, type UKPlace } from "@/components/uk-location-search";
import { Upload, Camera, AlertTriangle, CheckCircle2, Leaf, Cpu, Package, FileText, Layers, Wine } from "lucide-react";
...
export default function Scan() {
  ...
  const [place, setPlace] = useState<UKPlace | null>(null);
  ...
  function handleSubmitReport() {
    if (!scanWaste.data || !place) {
      toast({ title: "Missing location", description: "Please search for and select a UK location.", variant: "destructive" });
      return;
    }
    createReport.mutate(
      {
        data: {
          ...
          area: place.area,
          description: description || scanWaste.data.description,
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-muted-foreground">Upload a photo to classify waste with AI, then submit a report.</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={handleScan} disabled={!imageBase64 || scanWaste.isPending} className="w-full">
            {scanWaste.isPending ? "Analysing with AI..." : "Classify Waste"}
          </Button>
        </CardContent>
      </Card>
      ...
      {result && (
        <Card className="border-primary/30 bg-gradient-to-br from-background to-accent/20">
          <CardHeader>
      ...
      {result && !submitted && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Submit this report to your local council.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <UKLocationSearch
                ...
                placeholder="Search for a UK street, postcode, or landmark..."
              />
              {place && (
            ...
            <Button
              ...
              disabled={createReport.isPending || !place}
              className="w-full"
            >
              {createReport.isPending ? "Submitting..." : "Submit Report to Local Council"}
            </Button>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="border-green-200 bg-green-50/50">
          <CardContent className="pt-6 flex flex-col items-center text-center gap-3 py-8">
            ...
            <p className="text-sm text-green-700">
              Your report has been logged for {place?.area ?? "your area"} and sent to the local council.
            </p>
            <div className="flex gap-2 mt-2">
              ...
              <Button onClick={() => setLocation("/reports")}>View All Reports</Button>
            </div>
          </CardContent>
        </Card>
      )}
